...
<Note>
  The canonical App Store publish path is `asc publish appstore ... --submit --confirm`. Use `asc validate`, `asc submit status`, or `asc submit cancel` for readiness and submission lifecycle follow-up work. `asc release run` remains available as a deprecated compatibility pipeline.
</Note>
