...
The former `--state` update flag is deprecated and rejected because `state` is
response-only. Setting `--removed=true` also requires `--confirm`.

`asc review items view` and the compatibility command `asc review items-get`
are deprecated because API 4.4.1 does not provide
`GET /v1/reviewSubmissionItems/{id}`. Use
