...
* `duplicate` - **[experimental]** Duplicate a workflow under a new name
...
* `--id` - **[experimental]** Source workflow ID to copy (required)
* `--name` - **[experimental]** Name for the new workflow (required)
* `--description` - **[experimental]** Description for the new workflow (default: copied from the source workflow)
* `--enabled` - **[experimental]** Enable the new workflow immediately; the copy is created disabled by default
* `--output` - Output format: `json`, `table`, `markdown`
* `--pretty` - Pretty-print JSON output
