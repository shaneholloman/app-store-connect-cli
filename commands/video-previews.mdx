...
The `failures` field is additive and omitted when every upload succeeds. After a
later upload fails, asc attempts to delete every preview reserved during that
run before returning the receipt. When all cleanup deletes succeed, the receipt
keeps the asset IDs for diagnosis but marks those items `rolled-back`; the IDs
no longer refer to live previews. If a cleanup delete fails, the affected item
is marked `rollback-failed` and its asset ID remains a cleanup candidate. A
failed run does not reorder the preview set around a partially completed upload;
use the receipt's IDs for diagnosis and clean up only items that remain or are
marked `rollback-failed`.
