...
Resolve the exact product `resourceId` and its current version ID for every
product included in this release. Use `asc iap versions list` and
`asc subscriptions versions list` to find versions in
`PREPARE_FOR_SUBMISSION`. `READY_TO_SUBMIT` is an action item, not a passing
state.

Create one review submission, retain its returned ID, and add every affected
product version. A submission supports up to 200 items:
...
These review-submission commands do not support `--dry-run`; use the validation
report and exact product and version IDs as the pre-mutation check. Do not
submit the review submission until every intended item has been added.
