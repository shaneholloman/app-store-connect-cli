...
* `refactor(validate)`: Make `asc validate` the canonical App Store submission readiness command by adding shared content-rights and build-encryption checks
* `deprecate(submit)`: Mark `asc submit preflight` as a compatibility wrapper over `asc validate` with upgrade guidance
