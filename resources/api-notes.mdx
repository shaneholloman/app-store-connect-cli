...
Use `--paginate` with `asc analytics view --request-id ... --processing-date ...` to avoid missing instances on later pages. Without pagination, you may only receive partial data. The CLI forwards `--processing-date` as the server-side `filter[processingDate]` value. The deprecated `--date` compatibility flag was removed in 4.0.0; use `--processing-date`.
